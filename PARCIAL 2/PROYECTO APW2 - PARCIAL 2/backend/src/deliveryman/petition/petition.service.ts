...
export class PetitionService {
  ...
  private _sqlFilterOrder = (latitude: number, longitude: number):
    ...
    OR
    (
      (  (company.restricted = b'0') OR (company.restricted = b'1' AND rider IS NOT NULL)  )  AND  (p.status = :statusStarted AND ST_DistanceSphere(ST_GeomFromText('POINT(${latitude} ${longitude})'), p.location::geometry) <= :km)
    )`;
  ...
  async findNearPetitions(user: User, findPetitionsDto: FindPetitionsDto) {
    ...
    try {
      ...
      const petitions = await this._queryPetition(user.id)
  ...
  async findPetition(user: User, orderId: number) {
    try {
      const petition = await this._queryPetition(user.id)
  ...
  _queryPetition(idRider: number) {
    return this.orderRepository.createQueryBuilder('p')
      ...
      .leftJoinAndMapMany("company.companiesRider", "company_rider", 'rider', 'company.id = rider.companyId AND rider.userId = :idRider', { idRider })
  }
  ...
  async _getOrderAndNotify(orderId: number, deliverymanId: number) {
    const petition = await this._queryPetition(deliverymanId)
    ...
    if (petition) {
      const data = {
        ...
        "companyType": `${petition.store.company.type}`,
