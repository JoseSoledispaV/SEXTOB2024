...
import { Group } from '../../admin/group/entities/group.entity';
@Injectable()
export class MarketService {
  ...
  async findCompanies(storeMarketDto: StoreMarketDto) {
    const { limit = 100, offset = 0, latitude, longitude, categoryId, name } = storeMarketDto;
    try {
      ...
      if (name && companies.length > 0) {
        ...
        let companiesIdFilter: [] = [];
        companiesIdFilter = companies.reduce((ids, element, index) => {
          if (element.isOpen) ids.push(element.id)
          return ids;
        }, []);

        let productsFilter = [];
        productsFilter = await this.productViewRepository.createQueryBuilder('p')
          .where(`p.companyId IN (:...companiesIdFilter) AND UPPER(p.name) LIKE :name`, { companiesIdFilter, name: `%${name}%` })
          .getMany();

        let companiesIdFilterProduct: [] = [];
        companiesIdFilterProduct = productsFilter.reduce((ids, element, index) => {
          ids.push(element.companyId)
          return ids;
        }, []);

        if (companiesIdFilterProduct.length > 0) {
          companies = await query
            .where(`st.id IN (:...companiesIdFilterProduct)`, { companiesIdFilterProduct })
            .distinctOn(['id'])
            .take(limit)
            .skip(offset)
            .getMany();
          ...
          companies = await query
            .where(`st.id IN (:...companiesIdFilter) AND UPPER(st.name) LIKE :name`, { companiesIdFilter, name: `%${name}%` })
            .distinctOn(['id'])
            .take(limit)
            .skip(offset)
            .getMany();
        }
        return { companies, products: productsFilter };
      }
  ...
  async findOrders(user: User) {
    try {
      const orders = await this.orderRepository.createQueryBuilder('p')
        .select(['p', 'user.id', 'user.fullName', 'user.image', 'company.type', 'company.image', 'company.marker', 'store.id', 'store.name', 'store.address', 'store.location'])
  ...
  async findProductsByCompany(companyId: number, productMarketDto: ProductMarketDto) {
    const { limit = 100, offset = 0, groupId = 0 } = productMarketDto;
    try {
      ...
      if (groupId <= 0) {
        ...
        products = await this.productViewRepository.createQueryBuilder('p')
          .where(`p.companyId = :companyId AND p.groupId = :groupId`, { companyId, groupId })
          .take(limit)
          .skip(offset)
          .getMany();
      }

      const groupIds = products.filter(function (element) {
        return element.groupId;
      }).map(function (element) { return element.groupId; });

      if (groupIds.length > 0) {

        let groups = await this.groupRepository.createQueryBuilder('g')
          .where("g.id IN (:...groupIds)", { groupIds })
          .getMany();

        groups.unshift({ "id": 0, "name": "Todos" });
  ...
  async deliveryCost(user: User, companyIds: number[], costDeliveryMarketDto: CostDeliveryMarketDto) {
    ...
    const { latitude, longitude, fromlg, fromlt, isSumaryTaxi } = costDeliveryMarketDto;
    try {
      const query = this.companyViewRepository.createQueryBuilder('st');
      ...
      if (storeIds.length > 0) {
        ...
        if (isSumaryTaxi) {
          sqlCost =
            `${fromlg} AS fromlg, ${fromlt} AS fromlt, s.name, s.companyId, c.image, c.marker, s.id AS store_id, s.startupCost + ((ST_DistanceSphere(ST_GeomFromText('POINT(${latitude} ${longitude})'), 'POINT(${fromlt} ${fromlg})') / 1000) * s.costKm) AS deliveryFee`;
        } else {
          sqlCost =
            `s.location[1] AS fromlg, s.location[0] AS fromlt, s.name, s.companyId, c.image, c.marker, s.id AS store_id, s.startupCost + ((ST_DistanceSphere(ST_GeomFromText('POINT(${latitude} ${longitude})'), s.location::geometry) / 1000) * s.costKm) AS deliveryFee`;
        }
  ...
  async buy(user: User, orderMarketDto: OrderMarketDto) {
    ...
    try {
      ...
      this.notificationService.notifyOrder(order.id, order.store.id, orderMarketDto.location.x, orderMarketDto.location.y, data);
